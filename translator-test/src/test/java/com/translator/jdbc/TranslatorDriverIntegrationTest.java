...
public class TranslatorDriverIntegrationTest {
    ...
    public void setUp() throws Exception {
        ...
        translatorConnection = DriverManager.getConnection(
                translatorUrl, postgres.getUsername(), postgres.getPassword());
    ...
    public void testSelectWithFunction() throws Exception {
        ...
        ResultSet rs = statement.executeQuery(
                "SELECT IFNULL(`name`, 'N/A') AS `name` FROM `users` ORDER BY `id` LIMIT 1");
        Assert.assertTrue(rs.next());
        Assert.assertEquals("Alice", rs.getString("name"));
        rs.close();
    }
    ...
    public void testPreparedStatement() throws Exception {
        PreparedStatement pstmt = translatorConnection.prepareStatement(
    ...
    public void testNativeSql() throws Exception {
        ...
        String translated = translatorConnection.nativeSQL(
                "SELECT IFNULL(`name`, 'N/A') FROM `users` LIMIT 1");
        // 不应包含 IFNULL 或反引号
