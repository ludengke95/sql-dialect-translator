-- TPC-C 表结构（MySQL DDL）
-- 用于 SDTP 基准测试：直连 MySQL 建表，PostgreSQL 方言查询通过 SDTP 执行
-- 表直接创建在对应数据库（tpcc）下，无需 schema 前缀
-- 类型映射自 schema_pg.sql：NUMERIC->DECIMAL，TIMESTAMP->DATETIME，INTEGER->INT
-- 建表顺序按依赖关系排列（被外键引用的表先创建）
