...
if [ "$MODE" = "run-sdtp-mysql" ]; then
    ...
    python3 -c "
import os
content = open('/opt/benchmarksql/props.mysql.tpl').read()
for var in ['SDTP_HOST', 'SDTP_PORT', 'PG_DB', 'SDTP_USER', 'SDTP_PASSWORD', 'MYSQL_DRIVER']:
    content = content.replace('\${' + var + '}', os.environ.get(var, ''))
open('/opt/benchmarksql/props.mysql', 'w').write(content)
"
fi
...
if [ "$MODE" = "build-pg" ]; then
    ...
    echo "1. 创建 TPC-C 表结构..."
    ./runSQL.sh props.pg SQLFiles/tableCreates.sql
    
    echo "2. 多线程装载数据 (warehouses = 2)..."
    ./runLoader.sh props.pg numWarehouses 2
    
    echo "3. 创建索引与外键约束..."
    ./runSQL.sh props.pg SQLFiles/indexCreates.sql
    
    echo "=== TPC-C 数据构建完成 ==="

elif [ "$MODE" = "run-pg" ]; then
    echo "=== 开始 TPC-C 直连 PG 基线压测 (Baseline) ==="
    ./runBenchmark.sh props.pg > /opt/benchmarksql/run_pg.log 2>&1 || true
    echo "=== TPC-C 直连 PG 基线压测完成 ==="
    cat /opt/benchmarksql/run_pg.log | grep -i "tpmC" || true
...
while True:
    try:
        ...
        time.sleep(1)
"
    ./runBenchmark.sh props.mysql > /opt/benchmarksql/run_mysql.log 2>&1 || true
    echo "=== TPC-C 连 SDTP (MySQL) 压测完成 ==="
    cat /opt/benchmarksql/run_mysql.log | grep -i "tpmC" || true
